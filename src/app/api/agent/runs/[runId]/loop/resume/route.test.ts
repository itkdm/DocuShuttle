...
const harness = vi.hoisted(() => {
  const internalResult = {
    checkpoint: {
      ...
      iterations: 2,
      toolCallCount: 1,
      pendingResolution: { interactionId: "i-1", type: "approval", callId: "c-1", toolName: "apply", input: {}, decision: "approved" },
      messages: [{ role: "assistant", content: "", reasoning: "PRIVATE_REASONING_SENTINEL_123", toolCalls: [{ id: "c-1", name: "apply", input: {} }] }],
      finalText: "完成",
      permissionMode: "default",
    },
    events: [],
  };
  const runner = { resume: vi.fn(async () => internalResult) };
  const client = { from: () => ({ select: () => ({ eq: () => ({ single: async () => ({ data: { task_id: "task-1" }, error: null }) }) }) }) };
  return { internalResult, runner, client };
});

vi.mock("@/infrastructure/supabase/server", () => ({ requireSupabaseIdentity: vi.fn(async () => ({ client: harness.client })) }));
vi.mock("@/infrastructure/observability", () => ({ logger: { error: vi.fn(), info: vi.fn() } }));
...
const request = () => new Request("http://localhost/api/agent/runs/run-1/loop/resume", { method: "POST", body: JSON.stringify({ approval: "approved", interactionId: "00000000-0000-4000-8000-000000000001", callId: "c-1" }), headers: { "content-type": "application/json" } });

describe("resume public runtime boundary", () => {
  it("redacts private checkpoint fields from JSON", async () => {
  ...
  it("redacts private checkpoint fields from SSE result frames", async () => {
    const response = await PUT(new Request("http://localhost/api/agent/runs/run-1/loop/resume", { method: "PUT", body: JSON.stringify({ approval: "approved", interactionId: "00000000-0000-4000-8000-000000000001", callId: "c-1" }), headers: { "content-type": "application/json" } }), { params });
    const body = await response.text();
