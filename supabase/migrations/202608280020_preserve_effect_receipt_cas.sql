-- Preserve executor CAS ownership while making document effects and recovery receipts durable.
-- This is a forward migration; 019 remains unchanged.

drop function if exists public.commit_loop_document_version(uuid, bigint, uuid, text, text, text, text);
drop function if exists public.save_effect_receipt(uuid, jsonb);
...
begin
  ...
  return jsonb_build_object(
    ...
    'lockVersion', p_expected_run_version,
...
begin
  ...
  if found then
    ...
    return jsonb_build_object('receipt', v_existing.receipt, 'lockVersion', p_expected_lock_version);
  end if;
...
revoke all on function public.save_effect_receipt(uuid, bigint, jsonb) from public, anon;
grant execute on function public.save_effect_receipt(uuid, bigint, jsonb) to authenticated;
