-- The RPC returns a column named working_document_id. Qualify the table
-- column in the aggregate query so PostgreSQL does not confuse it with the
...
begin
  ...
  if p_role = 'template' or (p_role = 'example' and not v_has_template) then
    ...
    if p_role = 'template' or v_parent is null then
      select coalesce(max(dv.version_number), -1) + 1 into v_number
        from public.document_versions dv
        where dv.working_document_id = v_working and dv.owner_user_id = v_owner;
