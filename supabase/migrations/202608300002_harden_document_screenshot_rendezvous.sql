-- Bind document screenshot previews to the exact client-tool rendezvous.
-- This is a forward migration; 202608300001 is already deployed.

alter table public.assets add column if not exists document_revision text;
alter table public.assets add column if not exists page_number integer;

create unique index if not exists assets_preview_request_once_idx
  on public.assets(owner_user_id, task_id, provider_request_id)
  where kind = 'preview' and provider = 'document_surface' and provider_request_id is not null;
...
begin
  ...
  if p_interaction_type = 'client_tool' then
    if v_pending->>'toolName' <> 'capture_document_view' then raise exception 'CLIENT_TOOL_INTERACTION_MISMATCH'; end if;
    ...
    if v_pending->'input'->>'target' = 'visible' and v_asset.page_number is null then
      raise exception 'CLIENT_TOOL_ASSET_MISMATCH' using errcode = '22023';
    end if;
  end if;
