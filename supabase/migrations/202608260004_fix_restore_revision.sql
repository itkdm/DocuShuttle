...
declare
  ...
  v_created uuid;
begin
  select d.* into v_document from public.working_documents d join public.tasks t on t.id = d.task_id
  where d.task_id = p_task_id and d.owner_user_id = v_owner and t.owner_user_id = v_owner for update;
  ...
  select * into v_source from public.document_versions where id = p_source_version_id and owner_user_id = v_owner and working_document_id = v_document.id;
  ...
  values (v_owner, v_document.id, v_current.id, v_current.version_number + 1, 'restore', v_source.object_key, v_source.manifest_object_key, v_source.sha256, v_source.engine_version, v_source.validation, jsonb_build_array(jsonb_build_object('kind', 'restore', 'fromVersionId', v_source.id))) returning id into v_created;
  update public.working_documents set current_version_id = v_created, revision = v_document.revision + 1, updated_at = now() where id = v_document.id and owner_user_id = v_owner;
