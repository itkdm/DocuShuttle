...
export const createImageReplacementTools = (documents: DocumentEnginePort, working: WorkingDocumentAccessPort, assets: GeneratedAgentAssetStore, storage: PrivateObjectStoragePort, ownerUserId: string, taskId: string, session?: WorkingDocumentInspectionSession): readonly AgentTool[] => [{ name: "replace_document_image", description: "Replace one existing Word image with a generated private asset. This creates a new immutable document version and requires approval.", requiresApproval: true, inputSchema: schema, async execute(rawInput, context) {
  ...
  const inspected = session ? await session.inspect() : { bytes: current.bytes, inspection: await documents.inspect(current.bytes) }; const inspection = inspected.inspection;
  const node = inspection.manifest.nodes.find((candidate) => candidate.nodeId === input.targetNodeId && candidate.kind === "image");
  ...
  const mutation = await documents.mutate(inspected.bytes, { expectedRevision: input.expectedRevision, operations: [operation] }); const validated = await documents.validate(mutation.bytes); if (blockingPackageErrors(validated.diagnostics).length) throw new Error("DERIVED_DOCUMENT_VALIDATION_FAILED");
