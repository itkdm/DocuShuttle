...
function Activity({ activity, taskId, onApproval, deciding }: { activity: AgentActivity; taskId?: string; onApproval?: (choice: "approved" | "rejected") => void | Promise<void>; deciding: boolean }) {
  ...
  return <ToolActivityDisclosure state={activity.state} summary={<><span className={`agent-tool-item ${activity.state}`}><span className="agent-tool-icon">{icon}</span><span className="agent-tool-content"><span className="agent-tool-heading"><strong>{toolLabel(activity.name)}</strong><small>{detail}{activity.durationMs !== undefined && ` · ${activity.durationMs}ms`}</small></span></span></span></>}><div className="agent-tool-detail-content"><small><code>{activity.name}</code></small>{(activity.input !== undefined || activity.output !== undefined || activity.error) && <pre>{JSON.stringify(activity.errorDetails ?? activity.error ?? activity.output ?? activity.input, null, 2)}</pre>}{activity.state === "approval" && onApproval && <div className="agent-approval-actions"><button className="primary-small" onClick={() => void onApproval("approved")} disabled={deciding}>批准并执行</button><button onClick={() => void onApproval("rejected")} disabled={deciding}>拒绝</button></div>}</div></ToolActivityDisclosure>;
}
