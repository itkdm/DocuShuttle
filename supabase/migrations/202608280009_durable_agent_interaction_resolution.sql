-- Persist the user's interaction decision before clearing the runtime pause.
-- The resolution remains in the checkpoint until the runner consumes it.
...
begin
  ...
  if v_pending is null or v_pending = 'null'::jsonb then
    if v_checkpoint->'pendingResolution' = p_resolution then
      return v_checkpoint;
    end if;
