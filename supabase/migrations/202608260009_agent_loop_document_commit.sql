-- Commit a mutation executed by the provider-neutral Agent Loop.
-- This is separate from the legacy four-step promotion RPC so the loop can
-- decide its own semantic sequence while retaining the same CAS boundary.
