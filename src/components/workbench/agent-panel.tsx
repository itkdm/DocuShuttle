  ...
  return (
    <aside className="agent-panel" aria-label="纸上鸭 Agent">
      ...
      <div className="agent-content">
        {tab === "agent" && <>
      <div className="agent-message duck-message"><div className="message-meta"><span>鸭</span><strong>{mode === "production" ? "文档工作区" : "本地预览"}</strong><small>{mode === "production" ? "已保存" : "仅本地"}</small></div><p>{mode === "production" ? "我可以查看、修改并导出当前文档。每次修改都会先说明范围，再生成新的版本。" : "文件会在当前浏览器中检查和预览；连接工作区后即可运行完整 Agent 流程。"}</p></div>
          ...
          {stage !== "idle" && stage !== "awaiting" && <div className="progress-card"><div className="progress-top"><strong>{stage === "complete" ? "已完成并校验" : stage === "analyzing" ? "正在处理请求" : "正在创建新版本"}</strong><span>{pct}</span></div><div className="progress-track"><span style={{ width: pct }} /></div><small>{stage === "complete" ? "结果已确认，可以继续对话或下载文件" : "正在读取相关内容并执行下一步"}</small>{stage !== "complete" && <button className="cancel-run" onClick={onCancel}><StopCircle size={13} /> 取消任务</button>}</div>}
        </>}
        {tab === "plan" && <div className="plan-list"><div className="plan-summary"><strong>{loopResult ? "执行记录" : run ? "当前任务" : "尚未开始任务"}</strong><small>{loopResult ? `已处理 ${loopResult.checkpoint.iterations} 个步骤 · ${loopResult.checkpoint.status === "completed" ? "已完成" : loopResult.checkpoint.status === "failed" ? "未完成" : "进行中"}` : run ? "正在根据你的要求处理文档" : "发送一条目标后，这里会显示处理进度"}</small></div>{loopResult ? loopResult.events.map((event, index) => <div className="plan-row completed" key={`${event.type}-${index}`}><span className="plan-status"><Check size={13} /></span><div><strong>{event.type === "tool.completed" ? `已完成：${event.name}` : event.type === "tool.failed" ? `未完成：${event.name}` : event.type === "approval.required" ? `需要确认：${event.name}` : event.type === "assistant.message" ? "Agent 回复" : event.type}</strong><small>{event.text ?? event.error ?? "已处理"}</small></div></div>) : run ? run.steps.map((step, index) => { const title = step.kind === "analyze" ? "读取文档内容" : step.kind === "generate" ? "准备内容" : step.kind === "apply" ? "写入新版本" : "校验结果"; const detail = step.status === "completed" ? "已完成" : step.status === "running" ? "正在处理" : step.status === "failed" ? step.error?.message ?? "处理失败" : "等待处理"; return <div className={`plan-row ${step.status}`} key={step.id}><span className="plan-status">{step.status === "completed" ? <Check size={13} /> : step.status === "running" ? index + 1 : <Circle size={10} />}</span><div><strong>{title}</strong><small>{detail}</small></div>{step.status === "running" && <ChevronRight size={15} />}</div>; }) : <div className="plan-empty">开始任务后，这里会显示处理进度。</div>}</div>}
