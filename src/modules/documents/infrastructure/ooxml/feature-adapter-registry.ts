...
export function createDefaultFeatureAdapterRegistry(): FeatureAdapterRegistry {
  return new FeatureAdapterRegistry()
    ...
    .register({ featureId: "content-control", recognize: (context) => context.kind === "paragraph" && Boolean(context.contentControl), resolveCapabilities: () => [{ operation: "read", state: "supported" }, { operation: "replace-text", state: "guarded", reasonCode: "CONTENT_CONTROL_MUTATION_UNSUPPORTED", reason: "Content control boundaries and locking metadata require a dedicated feature adapter before writing." }] })
    .register({ featureId: "field", recognize: (context) => context.kind === "paragraph" && Boolean(context.field), resolveCapabilities: () => [{ operation: "read", state: "supported" }, { operation: "replace-text", state: "guarded", reasonCode: "FIELD_MUTATION_UNSUPPORTED", reason: "Field instruction and cached result must be edited as separate operations." }] })
    .register({ featureId: "tracked-revision", recognize: (context) => context.kind === "paragraph" && Boolean(context.revision), resolveCapabilities: () => [{ operation: "read", state: "supported" }, { operation: "replace-text", state: "guarded", reasonCode: "REVISION_MUTATION_UNSUPPORTED", reason: "Tracked revision boundaries require accept/reject semantics before text mutation." }] })
