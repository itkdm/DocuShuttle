...
import { AgentImageActivity, ImagePreview } from "./agent-image-activity";
...
function Turn({ turn, taskId, onApproval, deciding }: { turn: AgentThreadTurn; taskId?: string; onApproval?: (choice: "approved" | "rejected") => void | Promise<void>; deciding: boolean }) {
  ...
  const live = assistant.status === "running" || assistant.status === "awaiting_approval" || assistant.status === "awaiting_user" || assistant.status === "awaiting_client";
  ...
  useEffect(() => {
    const wasLive = previousStatus.current === "running" || previousStatus.current === "awaiting_approval" || previousStatus.current === "awaiting_user" || previousStatus.current === "awaiting_client";
