...
interface AgentPanelProps {
  ...
  onRun: (prompt: string, attachments?: readonly AgentImageAttachment[], lifecycle?: { accepted: () => void; failed: () => void }) => void | Promise<void>; onCancel: () => void | Promise<void>;
...
export function AgentPanel({ runtimeView, onCollapse, onRun, onCancel, workspaceReady, taskId, run, messages = [], activeEvents = [], historicalEvents = [], onLoopApproval, permissionMode, onPermissionModeChange, onLoadEarlier, hasEarlierMessages = false, loadingEarlierMessages = false, conversationLoading = false, approvalSubmitting = false }: AgentPanelProps) {
  ...
  const submit = async () => { if (submittingRef.current || inputBlocked || !workspaceReady || !taskId) return; const value = prompt.trim(); const submittedAttachments = attachments; if (!value && !submittedAttachments.length) return; submittingRef.current = true; setSubmitting(true); let settled = false; const release = (accepted: boolean) => { if (settled) return; settled = true; if (accepted) { setAttachments((current) => { const samePayload = current.length === submittedAttachments.length && current.every((item, index) => item.preview === submittedAttachments[index]?.preview); if (samePayload) { submittedAttachments.forEach((item) => URL.revokeObjectURL(item.preview)); return []; } return current; }); setPrompt((current) => current.trim() === value ? "" : current); } submittingRef.current = false; setSubmitting(false); }; if (submittedAttachments.reduce((total, item) => total + item.file.size, 0) > 40 * 1024 * 1024) { setAttachments((current) => current.map((item) => ({ ...item, error: "图片总大小不能超过 40MB" }))); release(false); return; } stickToBottomRef.current = true; setShowScrollToBottom(false); try { const uploaded = await Promise.all(submittedAttachments.map((item) => uploadBrowserImage(taskId, item.file))); void Promise.resolve(onRun(value, uploaded, { accepted: () => release(true), failed: () => release(false) })).catch(() => release(false)); } catch (error) { setAttachments((current) => current.map((item) => ({ ...item, error: error instanceof Error ? error.message : "图片上传失败" }))); release(false); } };
  return (
    <aside className="agent-panel" aria-label="纸上鸭 Agent">
      ...
      <div className="agent-composer">
        ...
        <textarea ref={promptRef} id="agent-prompt" value={prompt} onChange={(event) => setPrompt(event.target.value)} onPaste={addPastedImages} onKeyDown={(event) => { if (event.nativeEvent.isComposing) return; if (event.key === "Enter" && !event.shiftKey && !inputBlocked && workspaceReady && !submitting) { event.preventDefault(); void submit(); } }} placeholder={awaitingUserQuestion ? "请回答纸上鸭的问题…" : !workspaceReady ? "请先打开一份 DOCX" : inputBlocked ? "Agent 运行中，可先输入下一条提示词…" : "例如：把实验结论改得更专业，只改一个单元格…"} rows={2} readOnly={submitting} disabled={!workspaceReady} />
