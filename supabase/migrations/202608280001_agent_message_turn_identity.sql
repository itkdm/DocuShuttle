-- Agent Conversation is still in development. Every message belongs to the
-- immutable execution turn that produced it; this removes projection-time
-- guessing from run/message identity.
alter table public.messages add column if not exists turn_id uuid;
update public.messages set turn_id = run_id where turn_id is null and run_id is not null;
update public.messages set turn_id = gen_random_uuid() where turn_id is null;
alter table public.messages alter column turn_id set not null;
create index if not exists messages_turn_owner_idx on public.messages(turn_id, owner_user_id);
...
begin
  ...
  insert into public.messages(id, owner_user_id, conversation_id, turn_id, role, parts, run_id, message_key)
    values (p_user_message_id, v_owner, v_conversation, p_run_id, 'user', jsonb_build_array(jsonb_build_object('type','text','text',p_user_message)), p_run_id, p_user_message_id::text)
