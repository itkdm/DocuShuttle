...
Agent 面板以持久化事件为事实源组成单一 Execution Timeline。普通执行和审批恢复都通过 SSE 发送公开文本、工具生命周期、审批和终态；模型公开文本在没有工具边界时渲染为最终消息，工具前后的文本保留为阶段说明。工具输入/输出默认折叠，并在展示层做截断和敏感字段脱敏；模型的隐藏推理不会进入产品界面。
