...
function ImagePreview({ src, alt }: { src: string; alt: string }) {
  ...
  return <div className={`agent-image-preview ${state}`} role="img" aria-label={state === "error" ? "图片预览暂不可用" : alt}>
...
export function AgentImageActivity({ activity, taskId, onApproval, deciding }: { activity: Extract<AgentActivity, { type: "tool" }>; taskId?: string; onApproval?: (choice: "approved" | "rejected") => void | Promise<void>; deciding: boolean }) {
  ...
  return <div className={`agent-tool-item agent-image-activity ${activity.state}`}>
    <span className="agent-tool-icon"><ActivityIcon activity={activity} /></span>
    <div className="agent-tool-content">
      <div className="agent-tool-heading"><strong>{toolLabel(activity.name)}</strong><small>{detail}{activity.durationMs !== undefined && ` · ${activity.durationMs}ms`}</small></div>
