-- Repair the production function created from migration 004.
-- This forward migration intentionally preserves the capture provenance checks
-- and adds strict, internally consistent scroll result validation.
...
begin
  ...
  if p_interaction_type = 'client_tool' and v_pending->>'toolName' = 'capture_document_view' then
    ...
    if (select count(*) from jsonb_object_keys(p_resolution->'result')) <> 8
      ...
      or jsonb_typeof(p_resolution->'result'->'atBottom') <> 'boolean'
      or (p_resolution->'result'->>'moved')::boolean <> (abs((p_resolution->'result'->>'scrollTop')::numeric - (p_resolution->'result'->>'beforeScrollTop')::numeric) > 0.5)
      or (p_resolution->'result'->>'atTop')::boolean <> ((p_resolution->'result'->>'scrollTop')::numeric <= 0.5)
      or (p_resolution->'result'->>'atBottom')::boolean <> ((p_resolution->'result'->>'scrollTop')::numeric >= (p_resolution->'result'->>'maxScrollTop')::numeric - 0.5) then
      raise exception 'CLIENT_TOOL_RESULT_INVALID' using errcode = '22023';
    end if;
