-- Template/example semantics.
-- A template is the only source allowed to seed the Working Document. An
-- example is reference material and must never replace the editable document.
alter table public.source_files
  add column if not exists manifest_object_key text;

create index if not exists source_files_task_role_created_idx
  on public.source_files(task_id, role, created_at desc);
...
begin
  ...
  -- Only a template is editable. Examples and auxiliary files remain source
  -- material and are deliberately excluded from this import path.
  if p_role = 'template' then
    ...
    -- Every explicit template upload becomes a new immutable import version.
    -- This makes the UI's “replace template” action recoverable while an
    -- example upload still cannot move the Working Document pointer.
